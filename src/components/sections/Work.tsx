...
export const Work: React.FC = () => {
  ...
  return (
    <section id="work" className="py-24 bg-[#08090d] relative overflow-hidden">
      <Container size="lg">
        <SectionHeading
          badge="Verified Case Studies"
          title="Engineered for Category Dominance"
          subtitle="Explore real client growth outcomes across B2B SaaS, FinTech, Telehealth, and High-Growth Commerce."
        />
        ...
        <div className="flex flex-wrap items-center justify-center gap-2.5 mb-12">
          {tags.map((tag) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-semibold tracking-wider transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 ${
                filterTag === tag
                  ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30'
                  : 'bg-[#121520] text-slate-400 hover:text-white border border-[#212638]'
              }`}
            >
              {tag}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {filteredProjects.map((project) => (
            <motion.div
              ...
              className="rounded-3xl bg-[#0f1118] border border-[#212638] overflow-hidden group hover:border-indigo-500/50 transition-all duration-300 flex flex-col justify-between"
            >
              {/* Visual Card Header */}
              <div className={`p-8 bg-gradient-to-br ${project.imageBg} relative overflow-hidden flex flex-col justify-between min-h-[220px]`}>
                <div className="flex items-center justify-between z-10">
                  ...
                  <div className="w-9 h-9 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white group-hover:bg-white group-hover:text-black transition-all">
                    <ArrowUpRight className="w-5 h-5" />
                  </div>
                </div>

                <div className="z-10">
                  <span className="text-xs font-semibold text-indigo-300 block uppercase tracking-wider">{project.client}</span>
              ...
              <div className="p-6 bg-[#0f1118] flex flex-col justify-between flex-1 gap-6">
                <p className="text-sm text-slate-300 line-clamp-2">
                  <strong className="text-white">Challenge:</strong> {project.challenge}
                </p>

                {/* Key Metrics Row */}
                <div className="grid grid-cols-3 gap-3 p-3 rounded-2xl bg-[#141724] border border-[#212638]">
                  {project.results.map((res, idx) => (
                    <div key={idx} className="text-center">
                      <div className="text-base font-extrabold text-white font-display">{res.value}</div>
                ...
                {/* Card Action */}
                <div className="flex items-center justify-between pt-2 border-t border-[#1a1d2b]">
                  <div className="flex flex-wrap gap-1.5">
                    {project.tags.map((t, idx) => (
                      <span key={idx} className="text-[10px] px-2 py-0.5 rounded bg-[#171a26] text-slate-400 border border-[#212638]">
                        {t}
                      </span>
                    ))}
                  </div>
          ...
          {activeProject && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
              <motion.div
                ...
                className="bg-[#0f1118] border border-indigo-500/40 rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 shadow-2xl relative"
              >
                {/* Close Button */}
                <button
                  type="button"
                  onClick={() => setActiveProject(null)}
                  className="absolute top-5 right-5 p-2 rounded-full bg-[#191d2b] text-slate-300 hover:text-white border border-[#212638]"
                >
                  <X className="w-5 h-5" />
                </button>
                ...
                <div className="space-y-5">
                  <div className="p-4 rounded-xl bg-[#141724] border border-[#212638]">
